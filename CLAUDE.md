...
```
WorkflowExecutionService.ExecuteAsync(request)         ← 精簡編排器
  → ParseAndValidatePayload                            ← Schema v2 JSON only（SchemaJsonOptions.Default）
  → Hook(OnInput)                                      ← Schema.WorkflowHook polymorphic（CodeHook / WebhookHook）
  → WorkflowPreprocessor.PrepareAsync(Schema.WP)       ← 接收 Schema.WorkflowPayload，OfType<T> 分類
...
`AgentContextBuilder.ResolveToolsAsync(Schema.AgentNode, ...)` 合併：Tool Catalog（內建）+ MCP Servers + A2A Agents + HTTP APIs + OCR + Script
...
Layer 1: {{node:Agent-1}}       節點輸出（VariableContext.NodeOutputs + NodeNameMap）
Layer 2: {{var:counter}}        Workflow 變數（使用者定義，Code 節點可寫回）
Layer 3: {{env:API_URL}}        環境變數（AGENTCRAFTLAB_ 前綴 allowlist）
Layer 4: {{runtime:iteration_index}}  Runtime 變數（iteration/loop 等節點注入）
```

**解析器**：`IVariableResolver.Resolve(text, VariableContext)` — 單一入口，Flow + 7 個 NodeExecutor + FlowPlanValidator 都已接入。
...
6 個插入點：OnInput → PreExecute → PreAgent → PostAgent → OnComplete / OnError。Schema v2 polymorphic：`Schema.CodeHook`（TransformKind enum）/ `Schema.WebhookHook`（HTTP POST）。BlockPattern 支援 regex 攔截。`WorkflowHookRunner` 以 pattern match 分派。

### AI Build

自然語言 → FlowBuilderService → LLM 串流 → buildFromAiSpec（partial update 優先 → full rebuild fallback）。Tool ID 白名單由 ToolRegistryService 動態注入。NodeSpecRegistry 提供 Schema v2 nested 節點規格範例。LLM 直接輸出 Schema v2 shape（無 `data` wrapper），AiBuildPanel merge NODE_REGISTRY defaults 後套入畫布。

**規劃智能**：FlowPlannerPrompt Rule 13（工具自動推薦 — 分析即時資料的任務自動帶 search tool）+ Rule 14（parallel 後必須接 Synthesizer agent）。FlowPlanValidator 做確定性兜底（keyword 啟發式 warning）。
...
- **新節點類型**：(1) `NodeTypes` 加常數 (2) `NodeTypeRegistry` 加一行 (3) `NodeExecutorRegistry` 加 handler (4) `Schema/Nodes/` 加 sealed record (5) `NodeConfig.cs` 加 `[JsonDerivedType]` (6) JS `NODE_REGISTRY` + `types/workflow.ts` 加 NodeData (7) `NodeSpecRegistry` 加規格
