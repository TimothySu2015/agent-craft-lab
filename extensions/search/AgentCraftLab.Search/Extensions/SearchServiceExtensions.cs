...
public static class SearchServiceExtensions
{
        ...
        Action<SearchEngineOptions>? configureOptions = null)
    {
        ...
        services.TryAddSingleton<ISearchEngine, SqliteSearchEngine>();
    ...
    /// <summary>
    /// 註冊 PostgreSQL + pgvector 搜尋引擎（FullText tsvector + Vector HNSW + RRF 混合搜尋）。
    /// 需要 PostgreSQL 並啟用 pgvector 擴充。
    /// </summary>
    /// <param name="services">DI 容器。</param>
    /// <param name="connectionString">PostgreSQL 連線字串。</param>
    /// <param name="dimensions">向量維度（預設 1536，對應 text-embedding-3-small）。</param>
    /// <param name="configureOptions">搜尋引擎選項設定。</param>
    public static IServiceCollection AddCraftSearchPgVector(
        this IServiceCollection services,
        ...
        Action<SearchEngineOptions>? configureOptions = null)
    {
        ...
        services.AddSingleton<ISearchEngine>(
            new PgVectorSearchEngine(connectionString, dimensions, options));

        return services;
    }

    /// <summary>
    /// 初始化搜尋引擎資料庫（SQLite: 建立表格 + FTS5 虛擬表 + 清理過期索引）。
    /// 非 SQLite Provider（PgVector、MongoDB Atlas）會安全跳過。
    /// 應在應用程式啟動時呼叫。
    /// </summary>
    public static async Task InitializeSearchDatabaseAsync(this IServiceProvider serviceProvider)
    {
        using var scope = serviceProvider.CreateScope();
        var db = scope.ServiceProvider.GetService<SearchDbContext>();
        if (db is null) return; // 非 SQLite Search Provider，跳過 FTS5 初始化
