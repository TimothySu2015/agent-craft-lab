...
/// <summary>
/// ExecutionMemory 的 SQL Server 實作 — 記錄 Autonomous Agent 執行記憶，供跨 Session 學習。
/// 可選接入 CraftSearch（ISearchEngine）提供語義搜索，否則 fallback 到 Jaccard 相似度。
/// </summary>
public class SqlExecutionMemoryStore(
    IServiceScopeFactory scopeFactory,
    ISearchEngine? searchEngine = null,
    ILogger<SqlExecutionMemoryStore>? logger = null) : IExecutionMemoryStore
{
        ...
        string userId, string goalDescription, int limit = 5)
    {
        ...
        try
        {
            ...
            // 用搜索結果的 ID 從 SQL Server 取回完整 document
    ...
    private async Task IndexToSearchEngineAsync(ExecutionMemoryDocument memory)
    {
        ...
        {
            logger?.LogWarning(ex, "[Memory] Search indexing failed for {Id}", memory.Id);
        }
    }
}
