...
/// <summary>
/// ExecutionMemory 的 SQLite 實作 — 記錄 Autonomous Agent 執行記憶，供跨 Session 學習。
/// 可選接入 CraftSearch（ISearchEngine）提供 FTS5 語義搜索，否則 fallback 到 Jaccard 相似度。
/// </summary>
public class SqliteExecutionMemoryStore(
    IServiceScopeFactory scopeFactory,
    ISearchEngine? searchEngine = null,
    ILogger<SqliteExecutionMemoryStore>? logger = null) : IExecutionMemoryStore
{
    ...
    /// <summary>
    /// 語義搜索 — 有 CraftSearch 時用 FTS5（trigram + BM25），否則 fallback 到 Jaccard。
    /// </summary>
        ...
        string userId, string goalDescription, int limit = 5)
    {
        ...
        try
        {
            ...
            // 用搜索結果的 ID 從 SQLite 取回完整 document
    ...
    /// <summary>背景將記憶索引到 CraftSearch（FTS5），不阻塞主流程。</summary>
