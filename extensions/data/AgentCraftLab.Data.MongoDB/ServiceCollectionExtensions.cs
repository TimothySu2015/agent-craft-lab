...
{
    /// <summary>
    /// 使用 MongoDB 作為資料庫，替換已註冊的 SQLite Store（需先呼叫 AddSqliteDataProvider）。
    /// </summary>
        ...
        string databaseName = "agentcraftlab")
    {
        ...
        // Memory stores
        services.Replace(ServiceDescriptor.Singleton<IExecutionMemoryStore, MongoExecutionMemoryStore>());
        services.Replace(ServiceDescriptor.Singleton<IEntityMemoryStore, MongoEntityMemoryStore>());
        services.Replace(ServiceDescriptor.Singleton<IContextualMemoryStore, MongoContextualMemoryStore>());
        services.Replace(ServiceDescriptor.Singleton<ICheckpointStore, MongoCheckpointStore>());
        services.Replace(ServiceDescriptor.Singleton<ICraftMdStore, MongoCraftMdStore>());
        services.Replace(ServiceDescriptor.Singleton<IDataSourceStore, MongoDataSourceStore>());
        services.Replace(ServiceDescriptor.Singleton<IRefineryStore, MongoRefineryStore>());

        return services;
    }

    /// <summary>
    /// 使用 MongoDB Atlas 搜尋引擎，替換已註冊的 SQLite SearchEngine。
    ...
    private static void ValidateStoreCoverage(IServiceProvider serviceProvider)
    {
        ...
        foreach (var (iface, name) in storeInterfaces)
        {
            ...
            if (impl is not null && !impl.GetType().Namespace?.StartsWith("AgentCraftLab.Data.MongoDB") == true)
            {
                uncovered.Add(name);
            }
        }
