...
PostgreSQL で PgVector 拡張機能を使用している場合、内蔵の SQLite 検索エンジンを PgVector ベースのベクトル検索に置き換えることができます。`appsettings.json` に以下を追加：
...
有効化される機能：
- **PgVector ベクトル検索** — セマンティック類似性検索（embedding ベース）
- **全文検索** — PostgreSQL ネイティブ全文検索
- **RRF ハイブリッド検索** — ベクトルと全文の組み合わせ

> PgVector 拡張機能がインストールされていない場合、`UsePgVectorSearch` は無視されます。
...
各 KB は作成時に DataSource にバインドされます。検索リクエストが来ると、`SearchEngineFactory` が DataSourceId を参照し、対応する検索エンジンにルーティングします。これにより、同一システム内で複数の検索バックエンドを混在させることが可能です。
