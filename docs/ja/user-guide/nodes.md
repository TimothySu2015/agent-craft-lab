...
| `tool` | データノード | エージェントが呼び出せるビルトインツールを接続します |
...
| `upper` | 大文字に変換 |
| `lower` | 小文字に変換 |
| `script` | JavaScript サンドボックススクリプトを実行（AgentCraftLab.Script の有効化が必要） |

**適用シーン：** エージェント出力の後処理（JSON フィールドの抽出、テンプレートのフォーマット、正規表現によるクリーニング）、ノード間のデータ変換。
