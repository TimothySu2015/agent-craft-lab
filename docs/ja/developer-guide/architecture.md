...
| `AgentCraftLab.Script` | JS サンドボックスエンジン（Jint、IScriptEngine インターフェースで Roslyn/Python に置換可能） |
...
| `code` | Y | Y | | 確定的変換（9 種モード + JS サンドボックス） |
...
|    - Script（IScriptEngine、Jint JS サンドボックス）    |
|                                                       |
...
| スクリプトエンジンの置換 | `IScriptEngine` を実装 + DI 置換（Jint -> Roslyn -> Python） |
